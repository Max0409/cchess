...
from cchesslib.lookup import Winner, Token2Idx, flip_move, state_to_board_dict, replace_dict, mov_dir, move_to_str
...
def step(state, action):
    ...
    board[int(action[3])][int(action[2])] = board[int(action[1])][int(
        action[0])]
    ...
    for i in range(len(rows)):
        ...
        for letter in row:
            if letter.isalpha():
                ...
                planes[Token2Idx[letter] + int(letter.islower()) * 7][i][j] = 1
                j += 1
            else:
                j += int(letter)
    return planes
    ...
    if history and len(history) >= 5:
        ...
        for i in range(len(rows)):
            ...
            for letter in row:
                if letter.isalpha():
                    # 0 ~ 7 : upper, 7 ~ 14: lower
                    planes[Token2Idx[letter] + int(letter.islower()) * 7 +
                           14][i][j] = 1
                    j += 1
                else:
                    j += int(letter)
...
def state_to_fen(state, turns):
    '''transfer state to fen format'''
    fen = ''
    state = "".join(
        [state_to_board_dict[s] if s.isalpha() else s for s in state])
...
def get_catch_list(state: str, moves=None):
    catch_list = set()
    if not moves:
...
def be_catched(state: str, mov: str):
    i = int(mov[1])
    j = int(mov[0])
    position = [i, j]
    board = state_to_board(state)
...
def has_attack_chessman(state: str):
    '''check if board has attack chessman'''
